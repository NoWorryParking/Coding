...
{
    private string url = "";
    private string getDBParkings = "http://noworryparking.online/getparkings.php?";
        ...
        if (getFromGoogle)
        {
            ...
            if (dict.TryGetValue("results", out localParkingSpots))
            {
                ...
                foreach (var iter in temp)
                {
                    ...
                    parking.info.Add("lat", locationDict["lat"].ToString());
                    parking.info.Add("lng", locationDict["lng"].ToString());
                    StartCoroutine(DBManager.InsertParking(parking));
